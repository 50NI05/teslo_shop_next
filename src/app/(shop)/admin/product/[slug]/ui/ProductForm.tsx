...
export const ProductForm = ({ product, categories }: Props) => {
  ...
  const onSizedChanged = (size: string) => {
    ...
    <form onSubmit={ handleSubmit( onSubmit ) } className="grid px-5 mb-16 grid-cols-1 sm:px-0 sm:grid-cols-2 gap-3">
      ...
      <div className="w-full">
        ...
        <div className="flex flex-col mb-2">
          ...
          <div className="flex flex-wrap">

            {
              sizes.map(size => (
                <div 
                  key={size} 
                  onClick={ () => onSizedChanged(size) }
                  className={
                    clsx(
                      "p-2 border rounded-md mr-2 mb-2 w-14 transition-all text-center",
                      {
          ...
          { errors.sizes && <span className="text-red-500">Debes seleccionar al menos una talla.</span> }

          <input 
            type="hidden" 
            { ...register('sizes', { validate: value => value.length > 0 || 'Debes seleccionar al menos una talla' }) }  
          />
